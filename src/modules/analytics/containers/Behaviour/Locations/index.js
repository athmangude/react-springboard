...
export default class Locations extends PureComponent {
  ...
  constructor(props) {
    ...
    this.fetchBehaviourLocations = this.fetchBehaviourLocations.bind(this);
  ...
  async fetchBehaviourLocations() {
    ...
    try {
      const fetchBehaviourLocationsResult = await customerAnalyticsActions.fetchBehaviourLocations({ startTime: startDate, endTime: endDate }, selectedSegment.id, appliedFilters);

      const items = paginate(fetchBehaviourLocationsResult.data.Data, 0, null);

      this.setState({ items, data: fetchBehaviourLocationsResult.data.Data });
    } catch (exception) {
  ...
  render() {
    ...
    // const data = demoMode ? demoData : items;
    ...
    return (
      <TearedList
        isLoading={isLoading}
        title="My Locations"
        subtitle="How are my customers interactions per location?"
