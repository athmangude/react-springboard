...
import KeyDriverAnalysisGraph from './Graph';
import Themes from '../Themes';

export default class KeyDriverAnalysis extends Component {
  static propTypes = {
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (JSON.stringify(appliedFilters) !== JSON.stringify(newProps.appliedFilters) || startDate !== newProps.startDate || endDate !== newProps.endDate) {
      this.setState({ appliedFilters: newProps.appliedFilters, startDate: newProps.startDate, endDate: newProps.endDate }, () => this.fetchKeyDriverAnalysis());
    }
  }
  ...
  async fetchKeyDriverAnalysis() {
    ...
    try {
      const fetchKeyDriverAnalysisResult = await customerAnalyticsActions.fetchKeyDriverAnalysis({ startTime: startDate, endTime: endDate }, activeSegment, appliedFilters);
      this.setState({ data: fetchKeyDriverAnalysisResult.data.Data });
    } catch (exception) {
  ...
  render() {
    ...
    const { selectedThemes, isLoadingKeyDriverAnalysis, data } = this.state;
    return (
          ...
          <div style={{ width: '100%', paddingTop: 20 }}>
            {
              isLoadingKeyDriverAnalysis ? (
                <div style={{ backgroundColor: '#FFF', width: '100%', height: 100, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <KeyDriverAnalysisGraph data={data} themes={selectedThemes} colors={colors} width={width} />
              )
            }
          </div>
        </div>
