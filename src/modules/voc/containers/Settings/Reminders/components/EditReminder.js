...
const reminderTexts = [
  {
    label:
      "We notice that you have not yet completed your survey. Your feedback is important to us. Please reply to the message below to proceed with the survey.",
    value: 1
  },
...
export default class EditConversation extends Component {
  static propTypes = {
  ...
  constructor(props) {
    ...
    const selectedReminder = reminderTexts.find(
      reminder => reminder.label === conversation.reminderSettings.text
    );
    let activeScheduleType = "LATER";
    if (conversation.reminderSettings.sendNow) {
      activeScheduleType = "IMMEDIATELY";
    } else if (conversation.reminderSettings.recurring) {
    ...
    this.state = {
      ...
      payload: {
        reminder: {
          time: conversation.reminderSettings.time,
          text: conversation.reminderSettings.text,
          recurring: conversation.reminderSettings.recurring,
          sendNow: conversation.reminderSettings.sendNow,
          interval: conversation.reminderSettings.interval
        }
      },
      selectedReminderTextValue: selectedReminder
        ? selectedReminder.value
        : null,
      activeScheduleType,
      selectedDates: conversation.reminderSettings.time
        ? conversation.reminderSettings.time.map(time => moment(time))
        : [],
      mouseOver: null
    };
  ...
  onScheduleTypeChanged(selectScheduleType) {
    ...
    this.setState({
      activeScheduleType: selectScheduleType,
      payload: { reminder: { ...payload.reminder, recurring, sendNow } }
    });
  }
  ...
  onChange(event, { value, interval }) {
    const { payload } = this.state;
    this.setState({
      payload: { reminder: { ...payload.reminder, time: value, interval } }
    });
  }
  ...
  onChangeCustomReminder(e) {
    ...
    this.setState({
      payload: {
        reminder: { ...payload.reminder, [e.target.name]: e.target.value }
      }
    });
  }
  ...
  onReminderTextChanged(selectedReminder) {
    ...
    this.setState({
      selectedReminderTextValue: selectedReminder.value,
      showCustomReminderTextInput,
      payload: { reminder: { ...payload.reminder, text } }
    });
  }
  ...
  async onUpdate() {
    ...
    try {
      await surveyReminderActions.updateSurveyReminder(
        conversation.id,
        payload
      );
      alertActions.addAlert({
        type: "success",
        message: "Successfully updated survey reminder"
      });
  ...
  async onRemove() {
    ...
    try {
      ...
      alertActions.addAlert({
        type: "success",
        message: "Successfully removed survey reminder"
      });
      const fetchConversationsResult = await conversationActions.fetchConversations(
        currentPage,
        "active"
  ...
  render() {
    ...
    console.log("[reminderTexts]", reminderTexts);
        ...
        <div style={{ padding: "0 10px 0 10px" }}>
          ...
          >
            ...
            >
              ...
              >
                {extractInitials(conversation.title)}
              </div>
          ...
          >
            <div style={{ width: "100%", margin: "0 10px" }}>
              ...
              <MwambaDropDownSelect
                style={{ width: "100%" }}
                value={selectedReminderTextValue}
                options={reminderTexts}
                onChange={this.onReminderTextChanged}
                placeholder="Select a reminder text"
              />
              {showCustomReminderTextInput ? (
                <input
                  type="text"
                  ...
                  value={payload.reminder.text}
                  onChange={this.onChangeCustomReminder}
                  style={{
                    borderRadius: 0,
                    borderTop: "none",
                    borderRight: "none",
                    borderLeft: "none",
                    borderBottom: "1px solid rgba(191, 42, 43, 100)",
                    padding: "10px 5px",
                    width: "100%"
                  }}
                />
              ) : null}
              {payload.reminder.text && payload.reminder.text.length > 139 ? (
                <small
                  style={{
                    color: payload.reminder.text.length > 160 ? "red" : "orange"
                  }}
                >
                ...
                {scheduleTypes.map(option => (
                  <div
                    style={{
                      ...
                      justifyContent: "center",
                      backgroundColor:
                        option.scheduleType === activeScheduleType
                          ? "#487db3"
                          : "#d9d9d9",
                      color:
                        option.scheduleType === activeScheduleType
                    ...
                    onClick={() =>
                      this.onScheduleTypeChanged(option.scheduleType)
                    }
                    className={
                      option.scheduleType === activeScheduleType ? "active" : ""
                    }
                  >
                    <div
                      style={{
              ...
              >
                {activeScheduleType === "LATER" ? (
                  <SendLater
                    ...
                    form={{ runTime: selectedDates || null }}
                  />
                ) : activeScheduleType === "RECURRING" ? (
                  <Recurring
                    onChange={this.onChange}
                    form={{
                      runTime: selectedDates || null,
                      mode: conversation.reminderSettings.interval
                        ? conversation.reminderSettings.interval
                        : null
                    }}
                  />
                ) : (
