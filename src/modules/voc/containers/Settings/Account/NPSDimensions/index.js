...
class NPSDimensions extends Component {
  ...
  onTabSelected(selectedTab) {
    this.setState({ selectedTab, surveyId: null, availableNPSFIlters: [], appliedNPSFIlters: [] }, () => {
      this.fetchAvailableNPSFIlters();
      this.fetchAppliedNPSFilters();
    });
  }
  ...
  render() {
    ...
    const { surveyId, isFetchingAvailableNPSFilters, isFetchingAppliedNPSFilters, appliedNPSFIlters, availableNPSFIlters, isLoading } = this.state;
    ...
    return (
      <SettingsNavigationContainer EventHandler={this.props.EventHandler}>
        ...
        {this.state.selectedTab === 'Survey' && !surveyId ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', flexDirection: 'row', flexWrap: 'wrap' }}>
                  {
                      isFetchingAvailableNPSFilters && !availableNPSFIlters.length ? (
                        ...
                        <NPSDimensionPill stance="negative" key={i} filter={filter} surveyId={surveyId} tab={this.state.selectedTab} accountActions={this.props.accountActions} fetchAppliedNPSFilters={this.fetchAppliedNPSFilters} appliedNPSFIlters={appliedNPSFIlters} alertActions={this.props.alertActions} EventHandler={this.props.EventHandler} />
                      ))
                    }
                </div>
              </div>
