...
export default class Survey extends Component {
  static propTypes = {
    ...
    onTriggerReinvite: PropTypes.func,
  }
  ...
  onTriggerReinvite(conversation) {
    const { onTriggerReinvite } = this.props;
    onTriggerReinvite(conversation);
  }
        ...
        </div>
        {
          conversation.reinviteSettings ? (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', width: '100%' }}>
              ...
              <span>{moment.utc(conversation.reinviteSettings.date).local().format('LLLL')}</span>
            </div>
          ) : null
        }
        {
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', position: 'absolute', right: 0, top: 0, height: '100%', backgroundColor: 'rgba(255, 255, 255, 1)', padding: '0 10px' }}>
              {
                conversation.reinviteSettings ? (
                  <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end'}}>
                    <IconButton onClick={() => this.onTriggerReinvite(conversation)} icon="edit" style={{ margin: 0, padding: 6 }} />
                  </div>
                ) : (
                  <IconButton onClick={() => this.onTriggerReinvite(conversation)} icon="add" style={{ margin: 0, padding: 6 }} />
                )
              }
            </div>
          ) : null
