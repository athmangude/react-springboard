...
import styles from './WideConversationListItem.css.js';

const WideConversationListItemWrapper = styled.div`${styles}`;
...
class WideConversationListItem extends Component {
  static propTypes = {
  ...
  render() {
    ...
    return (
      <WideConversationListItemWrapper onMouseLeave={this.onMouseLeave} onMouseEnter={this.onMouseEnter} className="wide-conversation-list-item">
        <div style={{ width: '100%', padding: '5px 10px' }}>
          <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: windowDimensions.width > 480 ? 'row' : 'column', padding: '0px 20px', width: 'calc(100% - 100px)' }}>
              <div style={{ width: windowDimensions.width > 480 ? 'calc(100% - 130px)' : '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: windowDimensions.width > 480 ? 'flex-end' : 'flex-start', width: windowDimensions.width > 480 ? 130 : '100%' }}>
                {
          ...
          </div>
          {
            this.state.isMouseOver ? (
              <div className="hide-scrollbars" style={{ display: 'flex', flexDirection: 'row', width: '100%', overflow: 'scroll' }}>
                ...
                {
                  account.active ? (
                    <SurveyActionButton onClick={this.onDuplicateSurvey} disabled={this.state.isDuplicatingSurvey} loading={this.state.isDuplicatingSurvey} icon="library_books" text="Duplicate" />
                  ) : null
                }
                {
                  conversation.status !== 'DRAFT' ? (
                ...
                {
                  conversation.status === 'ACTIVE' && account.active ? (
                    <SurveyActionButton icon="lock" text="Deactivate" onClick={this.onDeactivateSurvey} disabled={this.state.isDeactivatingSurvey} loading={this.state.isDeactivatingSurvey} />
                  ) : conversation.status === 'INACTIVE' && account.active ? (
                    <SurveyActionButton icon="lock_open" text="Activate" onClick={this.onActivateSurvey} disabled={this.state.isActivatingSurvey} loading={this.state.isActivatingSurvey} />
                  ) : null
                }
                {
                  account.active ? (
        ...
        }
        {
          this.state.showSendSurveyComponent ? (
            <SendSurveyModal onHideSendSurveyModal={this.onHideSendSurveyModal} conversation={conversation} audiences={audiences} EventHandler={EventHandler} alertActions={alertActions} />
          ) : null
        }
        {
      ...
      </WideConversationListItemWrapper>
    );
  }
}

WideConversationListItem.propTypes = {
  conversation: PropTypes.object.isRequired,
  listType: PropTypes.string,
};

WideConversationListItem.contextTypes = {
  router: PropTypes.object.isRequired,
};

export default WideConversationListItem;
