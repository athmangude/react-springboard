...
const objectives = [
  ...
  },
  {
    key: 'CS',
    name: 'NPS Survey',
    description: "Net Promooter Score survey", // eslint-disable-line
    icon: 'tag_faces',
...
class EditSurvey extends Component {
  ...
  constructor(props) {
    ...
    this.decodeSurveyId = this.decodeSurveyId.bind(this);
  ...
  state = {
    ...
    validSubmit: null,
    showMoreTargetOptions: false,
    isFetchingDraftSurvey: true,
  }

  async componentWillMount() {
    const { conversationActions, dispatch, authentication } = this.props;
    const { router } = this.context;
    const conversationId = this.decodeSurveyId(router.route.match.params.id)[0].toString();

    try {
      const fetchConversationResult = await conversationActions.fetchConversation(conversationId);
      const conversationData = fetchConversationResult.data.Data;

      const country = authentication.user.countries.find((c) => c.id === conversationData.countryId);

      this.setState({
        form: {
          objective: conversationData.objective,
          questions: JSON.parse(conversationData.surveyJson),
          title: conversationData.title,
          maxRespondents: conversationData.maxRespondents,
          audience: conversationData.surveyMetadata.find((metadata) => metadata.name === 'panelId') ? Number(conversationData.surveyMetadata.find((metadata) => metadata.name === 'panelId').value) : null,
          age: { min: 0, max: 100 }, // TODO: discuss how we can get the age
          gender: conversationData.surveyMetadata.find((metadata) => metadata.name === 'gender') ? conversationData.surveyMetadata.find((metadata) => metadata.name === 'gender').value : null,
          scheduleType: conversationData.surveyMetadata.find((metadata) => metadata.name === 'scheduleType') ? conversationData.surveyMetadata.find((metadata) => metadata.name === 'scheduleType').value : null,
          runTime: conversationData.surveyMetadata.find((metadata) => metadata.name === 'runTime') ? JSON.parse(conversationData.surveyMetadata.find((metadata) => metadata.name === 'runTime').value) : null,
          retakable: conversationData.retakable,
          retakeInterval: conversationData.retakeInterval,
          retakeLimit: conversationData.retakeLimit,
          joincode: conversationData.joincode,
          participantTimeoutInterval: conversationData.participantTimeoutInterval,
          surveyTimeoutInterval: conversationData.surveyTimeoutInterval,
          defaultIncentiveAmount: conversationData.defaultIncentiveAmount === 0 ? null : conversationData.defaultIncentiveAmount,
          surveyMetadata: conversationData.surveyMetadata,
          language: conversationData.language,
          country: country.code,
        },
        conversation: conversationData,
        dimmed: false,
        isFetchingDraftSurvey: false,
        showMoreTargetOptions: conversationData.status === 'DRAFT',
      });
    } catch (exception) {
      ActivityHandler.handleException(dispatch, exception);
      this.setState({
        errorFetchingConversationData: true,
      });
    }
  }
  ...
  onChange(event, { name, value }, updateRemote = null) {
    ...
    if (name === 'questions' && conversation.status === 'ACTIVE') {
      return;
    }
  ...
  onViewSurveyResults() {
    ...
    setTimeout(() => {
      router.history.push(`/surveys/${this.encodeSurveyId(conversation.id)}/report/${conversation.objective.toLowerCase()}`);
    }, 1000);
    router.history.push(`/surveys/${this.encodeSurveyId(conversation.id)}/report/${conversation.objective.toLowerCase()}`);
  }
  ...
  onFilterChanged(audienceFilters) {
  ...
  async onActivate(sendNow = false) {
    const { conversationActions, dispatch, EventHandler, alertActions } = this.props;
    const { conversation, form } = this.state;
  ...
  async autoSave(logAction = false) {
    const { authentication, conversationActions, dispatch, alertActions, loggedInUserRole } = this.props;
    ...
    const { objective, questions, audience, scheduleType, maxRespondents, defaultIncentiveAmount, title, runTime, meta, joincode, poweredByMsurvey, retakable, retakeLimit, retakeInterval, participantTimeoutInterval, surveyTimeoutInterval, language, country } = form;
  ...
  async fetchAudiences() {
    const { audiencesActions, authentication, dispatch } = this.props;
  ...
  render() {
    const { configurations, loggedInUserRole, windowDimensions, audiencesActions, languagesActions, EventHandler, account, authentication } = this.props;
    const { width } = windowDimensions;
    const { successMessage, errorMessage, dimmed, isFetchingDraftSurvey, successfullyActivated, form, conversation, savingConversation, activatingConversation, isFormValid, validSubmit } = this.state;

    if (!loggedInUserRole || loggedInUserRole.name !== 'ADMIN') {
      return (
        <SimpleLayoutExtended>
          <GenericPagePlaceholder title="Restricted Access" text="Your account role does not allow you to edit a survey" width={width} />
        </SimpleLayoutExtended>
      );
    }
    ...
    if (!account.active) {
      return (
        <SimpleLayoutExtended>
          <GenericPagePlaceholder title="Restricted Access" text="Your account is deactivated. You cannot edit an existing survey" />
        </SimpleLayoutExtended>
      );
    }
          ...
          <Dimmer.Dimmable dimmed={dimmed} blurring style={{ borderRadius: 0, border: 'none', boxShadow: 'none' }}>
                ...
                isFetchingDraftSurvey ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', alignSelf: 'flex-start', position: 'fixed', top: '35%', left: 0, right: 0 }}>
                    <div style={{ border: 'solid 1px #d9d9d9', borderRadius: 8, boxShadow: '0px 0px 10px #d9d9d9', width: 400, backgroundColor: '#fff' }}>
                      <div style={{ padding: 10, margin: 10 }}>
                        <b style={{ color: '#3d4553', fontSize: 16 }}>Fetching Draft Survey!</b>
                      </div>
                      <div style={{ padding: 10, minHeight: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <p style={{ color: '#3d4553' }}>Please wait while we fetch your draft.</p>
                      </div>
                    </div>
                  </div>
                ) : successfullyActivated === true ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', alignSelf: 'flex-start', position: 'fixed', top: '35%', left: 0, right: 0 }}>
                    <div style={{ border: 'solid 1px #d9d9d9', borderRadius: 8, boxShadow: '0px 0px 10px #d9d9d9', width: 400, backgroundColor: '#fff' }}>
                      ...
                      <div style={{ padding: 10, minHeight: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <p style={{ color: '#3d4553' }}>{successMessage}</p>
                      </div>
            ...
            <Container style={{ marginBottom: 40 }}>
              <Row style={{ width: '100%', margin: '10px 0px' }}>
                <Objectives options={objectives} active={form.objective} configurations={configurations} onChange={this.onObjectiveChange} />
              </Row>
              <Form ref={(f) => this.form = f} onValidSubmit={this.onValidSubmit} onInvalidSubmit={this.onInvalidSubmit} onValid={this.onValid} onInvalid={this.onInvalid}>
                {
                  objectives.filter((objective) => objective.key === form.objective)[0].segments.map((Segment, i) => (
                    <Row key={i} style={{ width: '100%', margin: 0 }}>
                      <Segment {...this.state} audiencesActions={audiencesActions} languagesActions={languagesActions} EventHandler={EventHandler} countries={authentication.user.countries} onShowMoreTargetOptionsChanged={this.onShowMoreTargetOptionsChanged} onChange={this.onChange} onChangeMeta={this.onChangeMeta} onFilterChanged={this.onFilterChanged} conversation={conversation} onRetakableChanged={this.onRetakableChanged} onRetakeLimitChanged={this.onRetakeLimitChanged} onRetakeIntervalChanged={this.onRetakeIntervalChanged}  onChangeCountryChanged={this.onChangeCountryChanged} />
                    </Row>
                  ))
                }
                  ...
                  <div style={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', margin: 0, padding: 0 }}>
                    ...
                    */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap', flexDirection: 'row' }}>
                      <ActionButton text="Save&nbsp;Changes" icon="save" loading={savingConversation} disabled={savingConversation} onClick={this.onSaveDraft} primary />
                      {
                        conversation && conversation.status !== 'ACTIVE' ? (
                          <div>
                            <ActionButton type="submit" text="Activate" icon="toggle_on" loading={activatingConversation} disabled={!isFormValid || activatingConversation || savingConversation || conversation.surveyJson === 'null'} onClick={() => this.onActivate(false)} />
                            {
                              (form.objective !== 'BASIC' && form.audience !== null) ? (
                                <ActionButton type="submit" text="Activate&nbsp;and&nbsp;Send&nbsp;Now" icon="send" loading={activatingConversation} disabled={!isFormValid || activatingConversation || savingConversation || conversation.surveyJson === 'null'} onClick={() => this.onActivate(true)} />
                              ) : null
                            }
                          </div>
                        ) : null
...
export default withAuthentication(EditSurvey);
