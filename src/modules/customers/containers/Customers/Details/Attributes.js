...
const Attributes = ({ customer, customerSchema }) => {
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', marginBottom: 20 }}>
      <div style={{ width: '100%', height: 30, display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 13, fontWeight: 900, color: '#6d6e71' }}>Customer Attributes</div>
        {/* <div style={{ textTransform: 'uppercase', fontSize: 11, color: '#2574a6', cursor: 'pointer' }}>Manage Customer Attributes</div> */}
      </div>
      {
        rows.map((columns) => (
