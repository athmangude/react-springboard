...
export default class Summary extends Component {
  ...
  async fetchMetric() {
    ...
    const previousDateRange = {
      startDate: today.clone().subtract(1, identifier).startOf(identifier).format('YYYY-MM-DD HH:mm:ss'),
      endDate: today.clone().subtract(1, identifier).endOf(identifier).format('YYYY-MM-DD HH:mm:ss'),
    };

    const currentDateRange = {
      startDate: today.clone().startOf(identifier).format('YYYY-MM-DD HH:mm:ss'),
      endDate: today.clone().endOf(identifier).format('YYYY-MM-DD HH:mm:ss'),
    };
    ...
    try {
      ...
      const data = {
        previous: parseFloat(fetchMetricResult[0].data.Data.visits, 10),
        current: parseFloat(fetchMetricResult[1].data.Data.visits, 10),
      };
      this.setState({ data });
    } catch (exception) {
  ...
  render() {
    ...
    const { isLoadingMetrics } = this.state;
    const data = {
      previous: 56473,
      current: 78421,
    };
