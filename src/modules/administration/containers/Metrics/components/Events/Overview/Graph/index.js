...
export default class Graph extends Component {
  ...
  render() {
    const { data, chartType, paramOption } = this.props;
    const { left, right } = this.state;
          ...
          <ResponsiveContainer>
            {
              chartType === 'bar' ? (
                <BarChart barCategoryGap={5} data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey={paramOption} fill="url(#item)" maxBarSize={10} radius={[5, 5, 0, 0]} />
                </BarChart>
              ) : (
                <LineChart data={data} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Line type="linear" dataKey={paramOption} stroke="#f26b50" strokeWidth={2} dot={{ stroke: '#f26b50', strokeWidth: 2, r: 4 }} />
                </LineChart>
              )
            }
          </ResponsiveContainer>
