...
class CommDomains extends Component {
  static propTypes = {
  ...
  render() {
    const { commDomains, isLoading } = this.props;
    ...
    const currentOption = commDomains.find((option) => option.value === selected) || {};

    return (
      <div style={{ position: 'relative', margin: '2px 5px', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
        <ActionButton icon="settings_input_antenna" text={currentOption.text} onClick={this.onToggleMenu} style={{ border: 'solid 1px #e2e4eb' }} />
        {
          menuOpen ? (
...
export default enhanceWithClickOutside(CommDomains);
